...
export type JustProps<T extends object> = Pick<T, {
    [K in keyof T]: T[K] extends (...args: any[]) => any ? never : K;
}[keyof T]>;
export type ExtractRefProp<T> = NonNullable<{
    [P in keyof T]: T[P] extends DialogRef ? P : never;
}[keyof T]>;
export type ExtractData<T> = ExtractRefProp<T> extends never ? any : T[ExtractRefProp<T>] extends DialogRef<infer Data> ? Data : never;
export type ExtractResult<T> = ExtractRefProp<T> extends never ? any : T[ExtractRefProp<T>] extends DialogRef<any, infer Result> ? Result : never;
